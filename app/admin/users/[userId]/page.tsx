...
import { 
  ...
  CreditCard, User as UserIcon, Mail, Phone, MoreHorizontal,
  Shield, Activity, ExternalLink, Search, Filter, Download
} from "lucide-react"
    ...
    return (
      <NavbarLayout>
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8">
          ...
          <Link href="/admin" className="inline-flex items-center text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium">
          ...
          <div className="mb-6 flex justify-between items-center">
            ...
            {user && (
              <div className="hidden md:flex items-center space-x-2">
                <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${user.suspended ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300'}`}>
                  <span className={`w-2 h-2 rounded-full ${user.suspended ? 'bg-red-500 dark:bg-red-400' : 'bg-emerald-500 dark:bg-emerald-400'}`}></span>
                  {user.suspended ? 'Suspended' : 'Active'}
                </span>
                
                {user.subscriptionStatus && (
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(user.subscriptionStatus)} dark:bg-opacity-50 dark:border dark:border-opacity-20`}>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${user.role === 'admin' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300'}`}>
                  <span className="flex items-center gap-1">
                ...
                <div className="md:col-span-3">
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                      ...
                      <div className="group">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Subscription End Date</label>
                        <div className="relative">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      ...
                      <div className="group">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Payment Due ($)</label>
                        <div className="relative">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      ...
                      <div className="flex items-center py-2">
                        <label className="relative inline-flex items-center cursor-pointer">
                          ...
                          <span className="ms-3 text-sm font-medium text-gray-700 dark:text-gray-300">Suspend User Account</span>
                        </label>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.suspended ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300'}`}>
                              <span className={`w-2 h-2 rounded-full mr-1.5 ${user.suspended ? 'bg-red-500' : 'bg-emerald-500'}`}></span>
                              {user.suspended ? 'Suspended' : 'Active'}
                            </span>
                          </div>
                        </div>
                            ...
                            {user.subscriptionStatus ? (
                              <>
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(user.subscriptionStatus)}`}>
                                  <span className={`w-2 h-2 rounded-full mr-1.5 ${getStatusBgColor(user.subscriptionStatus)}`}></span>
                                  {user.subscriptionStatus.charAt(0).toUpperCase() + user.subscriptionStatus.slice(1)}
                                </span>
                        ...
                        <div>
                          ...
                          <div className="mt-1.5">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.role === 'admin' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300'}`}>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                      ...
                      <div className="space-y-4">
                        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                          ...
                          <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{stats?.invoiceCount || 0}</p>
                        </div>
                        ...
                        {stats?.recentInvoices && stats.recentInvoices.length > 0 ? (
                          <div className="space-y-3 mt-2">
                            {stats.recentInvoices.slice(0, 3).map((invoice) => (
                              <div key={invoice._id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                                <div className="flex justify-between items-start">
                                  <div>
                                    <p className="text-sm font-medium text-gray-900 dark:text-white">{invoice.invoiceNumber}</p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(invoice.createdAt).toLocaleDateString()}</p>
                                  </div>
                                ...
                                <div className="mt-2 flex justify-between items-center">
                                  ...
                                  <Link href={`/admin/invoices/${invoice._id}`} className="text-xs text-blue-600 dark:text-blue-400 hover:underline">
                                    View details
                                  </Link>
                                </div>
                              </div>
                            ...
                            <Link href="#" onClick={() => setActiveTab("invoices")} className="block text-center text-sm text-blue-600 dark:text-blue-400 hover:underline mt-3">
                              View all invoices
                            </Link>
                          </div>
                        ) : (
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                      ...
                      <div className="space-y-4">
                        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                          ...
                          <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{stats?.inventoryCount || 0}</p>
                        </div>
                        
                        <Link href="/admin/inventory" className="block text-center text-sm text-blue-600 dark:text-blue-400 hover:underline mt-5">
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                    ...
                    {stats?.loginHistory && stats.loginHistory.length > 0 ? (
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          ...
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {stats.loginHistory.map((login) => (
                              <tr key={login._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-center py-12">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No login activity found</h3>
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
