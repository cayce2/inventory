...
export default function NavbarLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
   ...
   useEffect(() => {
    ...
    // Only check auth status if not already logged in
    if (!isLoggedIn) {
      checkAuthStatus()
    }
  }, []) // Remove pathname dependency to prevent rechecks during navigation
  ...
  }, [isLoggedIn]) // Only depend on isLoggedIn, not pathname

  // Optimize the fetchNotifications function with useCallback to prevent recreation
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      setUnreadCount(response.data.filter((notification: Notification) => !notification.isRead).length)
    } catch (error) {
      console.error("Error fetching notifications:", error)
    }
  }, []) // No dependencies to prevent recreation during renders

  // Optimize the checkAdminStatus function with useCallback
  ...
  // Optimize the fetchUserEmail function with useCallback
  ...
  // Optimize the handleLogout function with useCallback
  ...
  // Optimize the checkAuthStatus function with useCallback
  ...
  }, [router, checkAdminStatus, fetchUserEmail, handleLogout])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-md border-b border-gray-200/50 shadow-sm">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex h-16 items-center justify-between">
            {/* Logo */}
            <Link href="/dashboard" className="flex items-center group transition-all duration-300 hover:scale-[1.02]">
              <Image
                src="/favicon.ico"
                alt="Logo"
                width={40}
              ...
              />
              <span className="text-2x font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
                STOCKSKE
              </span>
            </Link>
            ...
            <div className="flex items-center lg:hidden">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="mr-2 relative flex items-center justify-center w-10 h-10 rounded-full text-gray-700 hover:bg-gray-100 transition-all focus:outline-none"
              >
            ...
            <div className="hidden lg:flex items-center space-x-2">
              ...
              <div className="relative">
                <motion.button
                  ...
                  onClick={toggleNotifications}
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center justify-center w-10 h-10 rounded-full text-gray-700 hover:bg-gray-100 transition-all focus:outline-none relative"
                >
                  <Bell className="w-5 h-5" />
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 flex h-5 w-5">
                ...
                <AnimatePresence>
                  {isNotificationsOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-lg shadow-xl border border-gray-200 z-10 overflow-hidden"
                    >
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="max-w-[100px] truncate">{userEmail ? userEmail.split("@")[0] : "Profile"}</span>
      ...
      <AnimatePresence>
        {isNotificationsOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.2 }}
            className="fixed top-16 left-4 right-4 z-50 sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:max-w-md"
          >
            <NotificationPopup isOpen={isNotificationsOpen} onClose={() => setIsNotificationsOpen(false)} />
          </motion.div>
        )}
                ...
                <nav className="space-y-1 mt-6">
                  ...
                  {isAdmin && (
                    <Link
                      href="/admin"
                      className={`flex items-center px-4 py-3 text-base font-medium rounded-md ${
                        pathname === "/admin" ? "bg-purple-50 text-purple-600" : "text-gray-700 hover:bg-gray-100"
                      }`}
                    >
      ...
      {/* Main Content */}
      <main className="flex-1 container mx-auto px-4 py-8 pb-16 max-w-7xl">{children}</main>
